...
module.exports = createCoreService(UID_MENU_ITEM, ({ strapi }) => ({
  async bulkCreateOrUpdate(items, menuId) {
    const itemsToCreate = items.filter(
      (item) => !item.id || `${item.id}`.includes("create")
    );
    ...
    let createdItems = await Promise.all(
    ...
    itemsToCreate.forEach((item) => {
      ...
      if (hasNewParent) {
        const createdItem = createdItems.find(
          (_item) => _item.createId === item.id
        );
        const createdParent = createdItems.find(
          (_item) => _item.createId === item.parent.id
        );
    ...
    return [
      ...createdItems.filter(
        (item) => !updatedItems.find((_item) => _item.id === item.id)
      ),
      ...updatedItems,
    ];
  },
  ...
  async bulkDelete(items) {
    ...
    let lastItemsToDelete = items.filter((item) => {
      return (
        !item.parent || !items.find((_item) => item.parent.id === _item.id)
      );
    });
    ...
    const deleteLoop = (_items) => {
      return _items.map((item) => {
        const children = items.filter(
          (_item) => _item.parent && _item.parent.id === item.id
        );

        if (children.length) {
          return [item.id, ...deleteLoop(children)];
        }
  ...
  async getPopulation() {
    ...
    const customLayouts = get(layouts, "categoryItem", {});
    ...
    const population = fields.reduce((acc, { input }) => {
      ...
      if (input.type === "relation") {
        const relationPopulation = getService(
          "category-item"
        ).getRelationPopulation(input.name);
  ...
  async search(query) {
    const { match = "", size = 5, page = 0, taxonomy = "Categories" } = query;
    const filterNews = {
      // sort: conditional,
      start: Number(page) * Number(size),
      limit: Number(size),
      sort: {
        updatedAt: "desc",
      },
    ...
    const totalCatsMatch = await strapi.entityService.findMany(UID_MENU_ITEM, {
      filters: {
        title: { $contains: match },
        taxonomy: taxonomy,
      },
    });
    const totalPage = Math.ceil(totalCatsMatch.length / size);
    ...
    return {
      data: data,
      totalPage: totalPage,
      totalMatch: totalCatsMatch?.length,
    };
  },
}));
