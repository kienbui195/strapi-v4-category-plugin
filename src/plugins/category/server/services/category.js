...
module.exports = createCoreService(UID_MENU, ({ strapi }) => ({
  async create(params) {
    const { data } = params;
    const menuData = pick(data, ["title", "slug", "locale"], {});
    ...
    if (menuItemsData.length) {
      await getService("category-item").bulkCreateOrUpdate(
        menuItemsData,
        entity.id
      );
    }
  ...
  async update(id, params) {
    ...
    const entityToUpdate = await getService("category").findOne(id, {
    ...
    if (itemsToDelete.length) {
      await getService("category-item").bulkDelete(itemsToDelete);
    }

    // Next, create or update menu items.
    if (menuItemsData.length) {
      await getService("category-item").bulkCreateOrUpdate(menuItemsData, id);
    }
  ...
  async delete(id) {
    ...
    const itemsToDelete = await getService("category-item").findByRootMenu(id);
